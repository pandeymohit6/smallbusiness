...
@if (count($providers) > 0)
    ...
    {{-- Social Buttons --}}
    <div
        class="grid gap-3 {{ count($providers) > 2 ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-' . count($providers) }}">

        @foreach ($providers as $provider => $config)
            <a @if ($isRegisterPage) :href="accountType
                ...
                @else
                    ...
                    title="{{ __('Continue with :provider', ['provider' => $config['name']]) }}" @endif
                ...
                <iconify-icon icon="{{ $config['icon'] }}" class="text-lg"
                    @if ($provider === 'github') style="color: currentColor;" @endif></iconify-icon>
    ...
    {{-- Register only helper message --}}
    @if ($isRegisterPage)
        <p x-show="!accountType" class="mt-3 text-center text-sm text-red-500">
