                ...
                <ul class="nav-menus">
                    @if ($headerMenu && $headerMenu->items->count())
                        ...
                        @foreach ($headerMenu->items->whereNull('parent_id') as $menuItem)
                            ...
                            @else
                                <li class="nav-item {{ url()->current() == $menuItem->getUrl() ? 'active' : '' }}">
                    ...
                    <div class="country-selector-wrapper" x-data="{ open: false }" @click.outside="open = false"
                        ...
                        <button type="button" class="country-selector-btn" @click="open = !open"
                            ...
                            <span class="current-country">
                                @php
                                    $country = collect($availableCountries)->firstWhere('slug', $countryCode);
                                @endphp
                    ...
                    @auth
                        <div class="user-menu-wrapper" x-data="{ open: false }" @click.outside="open = false"
                            ...
                            <button type="button" class="user-menu-btn" @click="open = !open"
                                ...
                                <span
                                    style="width: 32px; height: 32px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 14px;">
                            ...
                            <div x-show="open" x-transition
                                ...
                                <div style="padding: 10px 0;">
                                    <a href="{{ route('seller.dashboard') }}"
                                        ...
                                        <i class="fas fa-user"></i> Seller Dashboard
                                    </a>
            ...
            @auth
                <div style="padding: 15px; border-top: 1px solid #eee; margin-top: 10px;">
                    ...
                    <a href="{{ route('profile.edit') }}"
                        style="display: block; padding: 10px; color: #0066cc; text-decoration: none; font-size: 13px; margin-bottom: 8px; border: 1px solid #e0e0e0; border-radius: 4px; text-align: center;">
                        👤 My Profile
                    </a>
                    <a href="{{ route('home') }}"
                        style="display: block; padding: 10px; color: #0066cc; text-decoration: none; font-size: 13px; margin-bottom: 8px; border: 1px solid #e0e0e0; border-radius: 4px; text-align: center;">
                    ...
                    <form method="POST" action="{{ route('logout') }}" style="display: block;">
                        @csrf
                        <button type="submit"
                            style="width: 100%; padding: 10px; color: #dc2626; background: #fee2e2; border: 1px solid #fca5a5; border-radius: 4px; font-size: 13px; font-weight: 600; cursor: pointer;">
                            🚪 Logout
                        </button>
                    </form>
                </div>
